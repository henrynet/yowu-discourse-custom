<?php 
	...
	if ( have_posts() ) {
		while ( have_posts() ) {
			...
			$disc_options = get_option('discourse');
			$disc_is_enable_sso = (isset( $disc_options['enable-sso'] ) && intval( $disc_options['enable-sso'] ) == 1);
			$disc_permalink = (string)$custom['discourse_permalink'][0];
			if(empty($disc_permalink)){
				$disc_permalink = esc_url($disc_options['url']);
			}
			if($disc_is_enable_sso){
  				$disc_permalink = esc_url($disc_options['url']) . '/session/sso?return_path=' . $disc_permalink;
			}
		} // end while
	} // end if
?>
			...
			<div class="col-md-12">
				...
				<div class="article-promotion col-md-10 col-md-offset-1 col-sm-12">
					...
					<div class="col-md-6 col-sm-6 no-padding promo-right">
						...
						<a class="btn btn-go-forum" href="<?php echo $disc_permalink; ?>"><i class="fa fa-comment"></i>前往會員專屬論壇</a>
						<?php else: ?>
